...
I<$N>, where N is a single digit, means group number N.
I<$f> means a filename where match was found.
All other characters, not preceeded by $, are output as is.
Number N can be surrounded by curly braces, this allows to use
a group number more than 9.
Symbol `f' can also be surrounded by curly braces.
