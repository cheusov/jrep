...
format. Within format string the symbol $ has a special meaning.

Doubled $ means the symbol $ itself.

Sequence I<$N>, where N is a single digit, means group number N.
Sequence I<$f> means a filename where match was found.  All other
characters, not preceeded by $, are output as is.  Number N can be
surrounded by curly braces, this allows to use a group number more
than 9.
Sequences I<$<> and I<$E<gt>> mean start and end markers respectively
(See options I<--marker-start>, I<--marker-end> and I<JREP_COLOR>
environment variable).
Symbols `f', `<' and '>' can also be surrounded by curly braces.

Within curly braces the following special
modifiers may be used.
I<n> for replacing LF (line feed) and backslash characters
with '\n' and '\\' sequences respectively.
I<N> for replacing LF with ASCII space.
I<s> for replacing a sequence of unicode spaces with a single one.
I<t> for trimming leading and trailing unicode spaces from the group.
I<b> for removing directory from filename.
I<c> for CSV quoting string.
I<C> same as I<-c> but always surround entry with double quotes.
I<e> for removing extension from filename.
Modifiers are applied from left to right.

Examples:
   jrep -h -O '${fb} ${0st}' '^.*BSD.*$' -r /path/to/directory
   jrep -h -O '${fc},${1c},${2C}' '(\S+)=(\S+)' *.txt
   jrep -O 'Do you $<like$> $1$<${2}$>? :-)' '([^\s.]+)(BSD)' *.txt
   jrep -8h -O '${1Nst}' '<b>(.+?)</b>' *.html
...
=item B<-d>,B<--directories> I<ACTION>
...
=item B<--exclude-dir> I<DIR_PATTERN>
...
This implies I<--color=never>.

=back

=head1 COPYRIGHT

Copyright 2016-2017 Aleksey Cheusov <vle@gmx.net>
