...
$N, where N is a single digit, means group number N.
Number N can be surrounded by curly braces, this allows to use
a group number more than 9. Within curly braces the following special
...
$f means a filename where match was found. Symbol `f' can also be
